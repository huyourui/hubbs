...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function syncToHidden() {
        ...
        html = html.replace(/\[hide\]/g, '<div class="hidden-content-block" data-type="hide">').replace(/\[\/hide\]/g, '</div>');
    ...
    function updateHiddenFields() {
        document.getElementById('image_ids').value = uploadedImages.map(function(img) { return img.id; }).join(',');
        document.getElementById('inserted_images').value = insertedImages.join(',');
    }
    ...
    function uploadAttachment(file) {
        if (file.size > maxAttachmentSize) {
            alert('附件 "' + file.name + '" 大小超过限制（最大 ' + formatFileSize(maxAttachmentSize) + '）');
            return;
        }
        ...
        xhr.open('POST', SITE_URL + '/api/upload.php');
        ...
        xhr.onload = function() {
            if (xhr.status === 200) {
                try {
                    ...
                    if (response.success) {
                        addAttachmentItem(response.attachment);
                    } else {
                        alert(response.error || response.errors || '上传失败');
                    }
                } catch (e) {
    ...
    function addAttachmentItem(attachment) {
        ...
        var item = document.createElement('div');
        item.className = 'attachment-item d-flex align-items-center justify-content-between p-2 bg-light rounded mb-1';
        item.dataset.id = attachment.id;
        item.innerHTML = '<div><i class="bi bi-file-earmark"></i> ' + escapeHtml(attachment.original_name) + ' <small class="text-muted">(' + formatFileSize(attachment.file_size) + ')</small></div><button type="button" class="btn btn-sm btn-outline-danger remove-attachment">删除</button>';
        ...
        item.querySelector('.remove-attachment').addEventListener('click', function() {
            ...
            .then(function(data) {
                if (data.success) {
                    item.remove();
                    uploadedAttachments = uploadedAttachments.filter(function(a) { return a.id !== attachment.id; });
                    updateAttachmentIds();
                } else {
                    alert(data.error || '删除失败');
                }
            ...
            <div class="card-body">
                ...
                <form method="POST" action="" enctype="multipart/form-data">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <a href="<?php echo SITE_URL; ?>/index.php" class="btn btn-outline-secondary">取消</a>
                    </div>
                </form>
            </div>
        </div>
