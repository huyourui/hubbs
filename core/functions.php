 ...
 * 渲染表情文本标记为emoji
 */
function render_emojis($content) {
    if (empty($content)) {
        return $content;
    }
    $emojis = get_wechat_emojis();
    foreach ($emojis as $name => $emoji) {
        $content = str_replace('[' . $name . ']', '<span class="wechat-emoji" title="' . $name . '">' . $emoji . '</span>', $content);
    }
    return $content;
}
...
function render_content($content) {
    ...
    // 允许的基本标签
    $allowedTags = '<p><br><strong><b><em><i><del><s><code><pre><blockquote><ul><ol><li><a><img><h1><h2><h3><h4><h5><h6><hr><table><thead><tbody><tr><th><td><div><span><font><span>';
    $content = strip_tags($content, $allowedTags);

    // 渲染微信表情
    $content = render_emojis($content);
